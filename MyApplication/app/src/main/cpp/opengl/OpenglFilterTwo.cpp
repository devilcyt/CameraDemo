...
OpenglFilterTwo::OpenglFilterTwo() {
...
void OpenglFilterTwo::onCreate() {
    ...
    if(program > 0){
        ...
        LOGD("TWO: vPosition = %d , fPosition = %d", vPosition, fPosition);
...
void OpenglFilterTwo::onChange(int width, int height) {
    surface_width = width;
...
void OpenglFilterTwo::draw() {
    LOGD("draw() :  two ");

    glClearColor(1.0f,0,0,1.0f); // 设置清屏颜色为白色
    ...
    switch(drawType){
        ...
        case 5:{
            LOGD("TWO: native draw picture ");
            glBindTexture(GL_TEXTURE_2D, textureId);

            if(twoPixels != NULL){
                LOGD("TWO: pixels is not null");
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pic_width, pic_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, twoPixels); // 传入图片资源 byte类型数组
            }
...
void OpenglFilterTwo::setMatrix(int screenWidth, int screenHeight, int picture_width, int picture_height) {

    initMatrix(matrix);

    float screen_scale = 1.0 * screenWidth / screenHeight ;
    float picture_scale = 1.0 * pic_width / pic_height;
    LOGD("mecer screen_scale = %f, picture_scale = %f", screen_scale, picture_scale);
    float  r;
    if(screen_scale < picture_scale){ // 图片高度缩放
        r = screenHeight / (1.0 * screenWidth / pic_width * screenHeight); // 得到高度缩放的比例
        shadowMatrix(-1, 1, -r, r, matrix);
    }else if (screen_scale > picture_scale){  // 图片宽度缩放
        r = screenWidth / (1.0 * screenHeight / pic_height * screenWidth); // 得到宽度缩放的比例
        shadowMatrix(-r, r, -1, 1, matrix);
    }
}

void OpenglFilterTwo::setImageByte(jint w, jint h, void *data) {
...
void OpenglFilterTwo::onDestroy() {
    LOGD("Two: onDestroy() %d, %d", vShader, fShader);
